...
//               <span className="text-[10px] uppercase tracking-[0.2em] text-slate-400 font-bold">AETHERIA Generated Report</span>
//            </div>
...
import { ReviewMetrics, GraphData } from '../types';
import { Sparkles, BarChart2, Network, X, RotateCcw, GitBranch, Copy, MessageSquare, Bell, AlertCircle, Zap, FileDown, ChevronRight } from 'lucide-react';
import KnowledgeGraph from './KnowledgeGraph';
import { ResearchTimeline } from './ResearchTimeline';
import ChartsViewer from './ChartsViewer';
import MoleculeViewer from './MoleculeViewer';
...
const DocumentViewer: React.FC<DocumentViewerProps> = ({ content, metrics, graphData, onRefine, onRegenerateGraph, onBackToProjects, onChatUpdate, chatHistory = [], onTimelineBranchClick, onTextSelected, onNodeExpand, onAddToChat, onDeleteNode }) => {
  ...
  // 2. Setup the Print Handler
  const handlePrint = useReactToPrint({
    contentRef: documentRef, // Points to the element we want to print
    documentTitle: "Aetheria_Research_Report",
    onAfterPrint: () => console.log("Print successful"),
  });

  // Sample alerts data (kept for your UI)
  const sampleAlerts = [
    { id: 1, title: 'Minocycline Shows Promising Results in Neuropathic Pain', source: 'PubMed', date: '7 Dec 2025', description: 'New clinical trial demonstrates minocycline efficacy.', link: 'https://pubmed.ncbi.nlm.nih.gov' },
    { id: 2, title: 'Novel Minocycline Derivatives Identified', source: 'Nature Neuroscience', date: '7 Dec 2025', description: 'Researchers identify novel derivatives.', link: 'https://nature.com' }
  ];
    ...
    try {
      ...
      const response = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=' + apiKey, {
        ...
        body: JSON.stringify({
          contents: {
            parts: [{
              text: `Based on this document content, generate exactly 5 thought-provoking questions.
              Document Content (first 2000 chars): ${content.substring(0, 2000)}
              Return ONLY a JSON array of 5 questions strings.`
            }]
          }
        })
      });
      ...
      console.error('Error generating questions:', error);
      setSuggestedQuestions(['What are the key findings?', 'How does this impact strategy?']);
    }
    setIsGeneratingQuestions(false);
  };
  ...
  const handleRegenerateGraph = async () => {
    if (!onRegenerateGraph) return;
    setIsRegeneratingGraph(true);
    try { await onRegenerateGraph(); } catch (e) { console.error(e); } finally { setIsRegeneratingGraph(false); }
  };
  ...
  const parseInlineMarkdown = (text: string, baseKey: string) => {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      if (match[1]) parts.push(<strong key={`${baseKey}-b-${key}`} className="font-bold text-slate-900">{match[1]}</strong>);
      else if (match[2]) parts.push(<em key={`${baseKey}-i-${key}`} className="italic text-slate-800">{match[2]}</em>);
      else if (match[3]) parts.push(<code key={`${baseKey}-c-${key}`} className="bg-slate-100 px-2 py-1 rounded text-teal-700 font-mono text-sm">{match[3]}</code>);
      key++;
      lastIndex = regex.lastIndex;
    }
  ...
  const parseTable = (lines: string[], startIndex: number): { table: JSX.Element; endIndex: number } | null => {
    ...
    if (idx >= lines.length || !lines[idx].includes('|') || !lines[idx].includes('-')) return null;

    const headerRow = currentLine.split('|').map(cell => cell.trim()).filter(c => c.length > 0);
    idx++;
    const bodyRows: string[][] = [];
    ...
    while (idx < lines.length) {
      ...
      if (cells.length === headerRow.length) bodyRows.push(cells);
      idx++;
    }
    ...
    const table = (
      <div key={`table-${startIndex}`} className="overflow-x-auto my-6 rounded-lg border border-slate-200 shadow-sm print:border print:shadow-none">
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-teal-50 print:bg-slate-100">
              {headerRow.map((header, i) => (
                <th key={`th-${i}`} className="border border-slate-200 px-4 py-3 text-left text-sm font-bold text-slate-800">
                  {parseInlineMarkdown(header, `th-${i}`)}
                </th>
  ...
  const formatMarkdown = (text: string | any) => {
    let contentText = typeof text === 'object' && text !== null ? (text.content || text.markdown || JSON.stringify(text)) : String(text || '');
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith('```')) {
        if (!inCodeBlock) { inCodeBlock = true; codeContent = ''; i++; continue; } 
        else {
          inCodeBlock = false;
          result.push(
            <pre key={`code-${i}`} className="bg-slate-900 text-slate-100 p-6 rounded-xl mb-6 overflow-x-auto font-mono text-sm border border-slate-700 shadow-lg print:bg-slate-100 print:text-slate-900 print:border-slate-300">
              <code>{codeContent}</code>
            </pre>
          );
          i++; continue;
      ...
      if (trimmedLine === '') { result.push(<div key={`space-${i}`} className="h-4"></div>); i++; continue; }
      if (trimmedLine.includes('|')) {
        const tableResult = parseTable(lines, i);
        if (tableResult) { result.push(tableResult.table); i = tableResult.endIndex; continue; }
      }

      if (line.startsWith('# ')) result.push(<h1 key={`h1-${i}`} className="text-5xl font-black text-slate-900 mt-16 mb-8 tracking-tight leading-tight">{parseInlineMarkdown(line.replace('# ', ''), `h1-${i}`)}</h1>);
      else if (line.startsWith('## ')) result.push(<h2 key={`h2-${i}`} className="text-3xl font-bold text-slate-800 mt-12 mb-6 pb-3 border-b-2 border-teal-500">{parseInlineMarkdown(line.replace('## ', ''), `h2-${i}`)}</h2>);
      else if (line.startsWith('### ')) result.push(<h3 key={`h3-${i}`} className="text-xl font-bold text-teal-700 mt-8 mb-4 flex items-center gap-2"><span className="w-1 h-6 bg-teal-500 rounded-full print:hidden"></span>{parseInlineMarkdown(line.replace('### ', ''), `h3-${i}`)}</h3>);
      else if (line.startsWith('#### ')) result.push(<h4 key={`h4-${i}`} className="text-lg font-bold text-slate-800 mt-6 mb-3">{parseInlineMarkdown(line.replace('#### ', ''), `h4-${i}`)}</h4>);
      else if (line.startsWith('- ')) result.push(<li key={`li-${i}`} className="ml-6 list-disc text-slate-700 mb-3 pl-3 marker:text-teal-500 leading-relaxed text-base">{parseInlineMarkdown(line.replace('- ', ''), `li-${i}`)}</li>);
      else if (line.startsWith('> ')) result.push(<blockquote key={`quote-${i}`} className="border-l-4 border-teal-500 pl-6 italic text-slate-700 my-8 bg-gradient-to-r from-teal-50 to-transparent py-4 rounded-r-lg">{parseInlineMarkdown(line.replace('> ', ''), `quote-${i}`)}</blockquote>);
      else result.push(<p key={`p-${i}`} className="text-slate-700 leading-8 mb-6 text-base font-light">{parseInlineMarkdown(line, `p-${i}`)}</p>);
      i++;
    }
    return result;
  };
  ...
  return (
    <div className="h-full flex flex-col relative bg-[#f8fafc]">
      ...
      {/* 3. CSS for Print - Hides everything that isn't the document */}
      <style jsx global>{`
        @media print {
            body * {
                visibility: hidden;
      ...
      {/* Toolbar */}
      <div className="h-20 px-8 flex items-center justify-end sticky top-0 z-20 glass border-b border-white/50 shadow-sm transition-all duration-300 no-print">
        <div className="flex items-center gap-2">
          {/* Download/Print Button */}
          <button 
            onClick={() => handlePrint()}
            className="flex items-center gap-2 px-4 py-2.5 text-slate-600 hover:text-red-700 hover:bg-white rounded-full transition-all text-xs font-bold border border-transparent hover:border-slate-200 hover:shadow-sm"
          >
            <FileDown className="w-4 h-4" />
            Download PDF
          </button>
          
          <button onClick={() => setShowGraph(true)} className="flex items-center gap-2 px-4 py-2.5 text-slate-600 hover:text-teal-700 hover:bg-white rounded-full transition-all text-xs font-bold border border-transparent hover:border-slate-200 hover:shadow-sm">
            <Network className="w-4 h-4" /> Visual Graph
          </button>

          <button onClick={handleGenerateCharts} disabled={isLoadingCharts} className="flex items-center gap-2 px-4 py-2.5 text-slate-600 hover:text-amber-700 hover:bg-white rounded-full transition-all text-xs font-bold border border-transparent hover:border-slate-200 hover:shadow-sm disabled:opacity-50">
            {isLoadingCharts ? <RotateCcw className="w-4 h-4 animate-spin" /> : <BarChart2 className="w-4 h-4" />}
            {isLoadingCharts ? 'Generating...' : 'Generate Charts'}
          </button>

          <button onClick={() => setShowMolecule(true)} className="flex items-center gap-2 px-4 py-2.5 text-slate-600 hover:text-cyan-700 hover:bg-white rounded-full transition-all text-xs font-bold border border-transparent hover:border-slate-200 hover:shadow-sm">
            <Zap className="w-4 h-4" /> Molecular Structure
          </button>

          <button onClick={() => setShowTimeline(!showTimeline)} className={`flex items-center gap-2 px-4 py-2.5 rounded-full transition-all text-xs font-bold border ${showTimeline ? 'bg-purple-50 text-purple-700 border-purple-100 shadow-inner' : 'text-slate-600 border-transparent hover:bg-white hover:border-slate-200 hover:shadow-sm'}`}>
            <GitBranch className="w-4 h-4" /> Research Timeline
          </button>
          
          <div className="h-6 w-px bg-slate-200 mx-2"></div>
          
          <div className="relative">
            <button onClick={() => setShowAlerts(!showAlerts)} className={`p-2.5 rounded-full transition-all relative ${showAlerts ? 'text-red-600 bg-red-50' : 'text-slate-500 hover:text-red-600 hover:bg-white'} hover:shadow-sm`}>
              <Bell className="w-4 h-4" />
              {sampleAlerts.length > 0 && <span className="absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full animate-pulse"></span>}
            </button>
            ...
            {showAlerts && (
              <div className="absolute right-0 top-12 w-96 bg-white border border-slate-200 rounded-lg shadow-2xl z-50 max-h-[500px] overflow-y-auto">
                ...
                <div className="divide-y divide-slate-100">
                  {sampleAlerts.map((alert) => (
                    <button key={alert.id} className="w-full text-left px-4 py-3 hover:bg-slate-50 transition-colors group cursor-pointer">
                      <div className="flex items-start gap-2">
                        <div className="mt-1 w-2 h-2 rounded-full bg-red-500 flex-shrink-0 group-hover:bg-red-600"></div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-slate-900 group-hover:text-teal-700">{alert.title}</p>
      ...
      <div className="flex-1 overflow-y-auto scroll-smooth relative px-4 md:px-0">
        <div 
          ref={documentRef}
          id="printable-content" // ID for Print CSS targeting
          className="max-w-[850px] mx-auto bg-white min-h-[1100px] shadow-[0_8px_30px_rgba(0,0,0,0.04)] my-12 p-16 md:p-20 rounded-none md:rounded-xl border border-slate-100 animate-fade-up"
          onMouseUp={handleTextSelection}
          onTouchEnd={handleTextSelection}
        >
      ...
      {/* Modals & Overlays (Wrapped in no-print to be safe) */}
      <div className="no-print">
        {showSelectionMenu && selectedText && (
            <div ref={selectionMenuRef} className="fixed z-50 bg-white border border-slate-200 rounded-lg shadow-lg p-2 flex gap-2" style={{ left: `${menuPosition.x}px`, top: `${menuPosition.y}px`, transform: 'translateY(-120%)' }}>
            <button onClick={handleAddToChat} className="px-3 py-2 text-sm font-medium text-slate-700 hover:bg-teal-50 rounded flex items-center gap-1.5"><MessageSquare className="w-4 h-4" /> Add to Chat</button>
            <button onClick={handleSaveToNotes} className="px-3 py-2 text-sm font-medium text-slate-700 hover:bg-amber-50 rounded flex items-center gap-1.5"><Copy className="w-4 h-4" /> Save to Notes</button>
            <button onClick={() => setShowSelectionMenu(false)} className="p-1 text-slate-400 hover:bg-slate-100 rounded"><X className="w-4 h-4" /></button>
            </div>
        )}

        {showMetrics && metrics && (
            <div className="absolute right-0 top-20 bottom-0 w-[400px] bg-white/95 backdrop-blur-xl border-l border-slate-200 shadow-2xl p-8 overflow-y-auto z-30">
                <div className="flex justify-between items-center mb-10">
                    <h3 className="font-bold text-xl text-slate-800">Quality Assurance</h3>
                    <button onClick={() => setShowMetrics(false)}><X className="w-5 h-5 text-slate-400" /></button>
                </div>
                <div className="mb-10 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                    <span className="text-4xl font-black text-teal-600">{metrics.overallScore}</span>
                    <div className="w-full bg-slate-200 rounded-full h-3 mt-2"><div className="bg-teal-500 h-3 rounded-full" style={{ width: `${metrics.overallScore}%` }}></div></div>
                </div>
                {/* Metric rows omitted for brevity, logic matches original */}
            </div>
        )}

        {showGraph && graphData && (
            <div className="absolute inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-8">
                <div className="bg-white w-full h-full max-w-7xl max-h-[85vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col">
                    <div className="p-6 border-b border-slate-100 flex justify-between">
                        <h3 className="font-bold text-lg">Knowledge Graph</h3>
                        <button onClick={() => setShowGraph(false)}><X className="w-6 h-6" /></button>
                    </div>
                    <div className="flex-1 bg-slate-50 relative">
                        <KnowledgeGraph data={graphData} onNodeExpand={onNodeExpand} onAddToChat={onAddToChat} onDeleteNode={onDeleteNode} />
                    </div>
                </div>
            </div>
        )}
        ...
        {showTimeline && (
            <div className="absolute inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-8">
                <div className="bg-white w-full h-full max-w-2xl max-h-[85vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col">
                    <div className="p-6 border-b flex justify-between"><h3 className="font-bold">Research Timeline</h3><button onClick={() => setShowTimeline(false)}><X className="w-6 h-6" /></button></div>
                    <div className="flex-1 bg-slate-50 overflow-y-auto p-6">
                        <ResearchTimeline chatHistory={chatHistory} onBranchClick={async (q, i) => { setSelectedTimelineIndex(i); if (onTimelineBranchClick) await onTimelineBranchClick(q, i); }} selectedIndex={selectedTimelineIndex} />
                    </div>
                </div>
            </div>
        )}

        {showCharts && <ChartsViewer charts={chartData} onClose={() => setShowCharts(false)} />}
        {showMolecule && <MoleculeViewer onClose={() => setShowMolecule(false)} />}
      </div>
    </div>
  );
};
